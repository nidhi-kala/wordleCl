document.addEventListener("DOMContentLoaded", () => {
  ...
  let allWords = ['rails','class','index','react', 'event','views', 'crime', 'bring', 'songs', 'nidhi', 'ishan', 'rohan', 'seema']
  let word_of_the_day = "nidhi";
  let answer = word_of_the_day.split('');
  let currentAttempt = [];

  /* when enter is pressed:
  1) check currentATTEMPt has required miber of letters: 5;
  ...
  */
  ...
  function deleteHandler(game) {
    let previousSquare = document.getElementById(game['attempt'] + "" + game['letter']);
    if( game['letter'] == 4 && previousSquare.innerHTML != '') {
      previousSquare = document.getElementById(game['attempt'] + "" + game['letter'])
    } else if (game['letter'] == 0) {
      return;
    } else {
      previousSquare = document.getElementById(game['attempt'] + "" + (game['letter'] - 1))
      game['letter'] -= 1
    }
  ...
  for (let i = 0; i < keys.length; i++) {
    keys[i].onclick = ({target}) => {
      ...
      let currentSquare = document.getElementById(game['attempt'] + '' + game['letter']);
